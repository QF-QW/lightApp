...
public class SocietiesServlet extends javax.servlet.http.HttpServlet {
    @Override
...
//
            ...
            List<org.apache.commons.fileupload.FileItem> fileItems = null;
            try {
                ...
                for(org.apache.commons.fileupload.FileItem fileItem : fileItems){/*之后每次获取的都是单个文件*/
                    ...
                    if(fileItem.isFormField()){/*判断是不是普通的表单，就是普通的请求参数*/
                        ...
                        switch (fieldName){
                            case "societiesName" : societies.setSocietiesName(value);break;
                            case "introduction" : societies.setIntroduction(value);break;
                            case "createId" : societies.setCreateId(Integer.parseInt(value));break;
                            case "room_coordinate" : societies.setRoomCoordinate(value);break;
                            default:
                                System.out.println("出现意外的参数");
                        }
                        ...
                        if(!(name.trim()).equals("")){
                            ...
                            societies.setHeaderUrl(saveUrl);
                        }
            ...
            new SocietiesServerImpl().createSocieties(societies);
        ...
        }else if("getSocietiesList".equals(req.getParameter("type"))) {
            try {

                SocietiesServerImpl societiesServer = new SocietiesServerImpl();
                JSONObject result = societiesServer.getSocietiesList(Integer.parseInt(req.getParameter("page")));
            ...
            int societiesId = Integer.parseInt(req.getParameter("societiesId"));

            User user = (User)req.getSession().getAttribute("user");

            SocietiesServerImpl societiesServer = new SocietiesServerImpl();

            if (societiesServer.searchJoinServer(societiesId, user.getId())){
            ...
            societiesServer.joinServer(societiesId, user.getId());
